...
class Peserta extends CI_Controller {
    ...
    $page = $this->page_model->get_data_page(1);
	...
	public function proses_daftar()
	{
		...
		$check_nidn_nip = $this->peserta_model->check_nidn_nip_peserta($nidn_nip);
		if($check_nidn_nip > 0)
		{
			$this->session->set_flashdata('message','Maaf NIDN / NIP Anda sudah terdaftar.');
    ...
    $page = $this->page_model->get_data_page(22);
    foreach($page->result() as $p){
    	$data['title'] = $p->title;
      $data['keywords'] = $p->keywords;
      $data['description'] = $p->description;
		...
		$this->load->view('upload_bukti_pembayaran_view', $data);
	}
